# Claude Desktop 中文补丁 · 修复版

> **这是一个修复版 / 增强版。** 原项目 [javaht/claude-desktop-zh-cn](https://github.com/javaht/claude-desktop-zh-cn) 在当前 Claude Desktop 版本 + 3P 模式下**无法正常使用**(白屏、`SyntaxError`、兼容性差、无人维护),本仓库基于它修复,让汉化在当前环境能稳定跑起来,并新增一键管理器。
>
> 本仓库只介绍**我们修复了什么**,不介绍原版机制;想看原版怎么工作的,请直接去原作者仓库。

> **⚠️ 测试范围声明**：本项目在 **Windows** 上开发并测试通过(Claude Desktop + 3P 模式)。**macOS 未经过我们测试**,原版的 macOS 脚本沿用原项目,但我们无法保证其在 Mac 上可用。Mac 用户请谨慎使用,如有问题可先反馈给我们。

## 我们修复了什么

- **修复 3P 模式白屏**：原版批量修改约 137 个 JS 文件导致 `SyntaxError` 和白屏,本版恢复这些 JS,改用"合并中文 JSON + 只注册一处 `zh-CN` 白名单"的安全方案,不再破坏页面。
- **补全配置路径**：原版漏掉新版 Claude 的 `LOCALAPPDATA` 配置位置,导致找不到配置;本版补全。
- **重写安全备份机制**：原版把备份写进受保护的安装目录,容易失败;本版改到用户目录按版本单独备份,拒绝旧版本文件覆盖新版本。
- **解决文件权限问题**：本版会临时取得必要文件的写权限、操作后恢复,不会被权限卡死。
- **新增一键管理器**：双击 `Claude简体中文管理器.cmd`,支持一键汉化 / 修复汉化 / 卸载汉化 / 查看日志,自动识别 Claude 版本,Claude 更新后提示重新适配。
- **不碰关键配置**：不修改 `Claude.exe`、`app.asar`,不改 API 密钥、Gateway 和 3P 模型配置。
