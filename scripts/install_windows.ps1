...
function Get-ClaudeConfigPaths {
    ...
    if ($env:LOCALAPPDATA) {
        $configPaths += Join-Path $env:LOCALAPPDATA "Claude\config.json"
        $configPaths += Join-Path $env:LOCALAPPDATA "Claude-3p\config.json"
    }
...
function Get-BackupRoot {
    ...
    $localData = if ($env:CLAUDE_ZH_ORIGINAL_LOCALAPPDATA) {
        $env:CLAUDE_ZH_ORIGINAL_LOCALAPPDATA
    }
    else {
        $env:LOCALAPPDATA
    }
    if (-not $localData) {
        throw "无法确定当前用户的 LocalAppData，不能创建安全备份。"
    }

    $appPath = Get-ClaudeAppPathFromResources $ResourcesPath
    $installRoot = Split-Path -Parent $appPath
    $installId = Split-Path -Leaf $installRoot
    if (-not $installId) {
        $installId = "Claude-unknown"
    }

    return Join-Path $localData (Join-Path "ClaudeZhCnMinimal\backups" $installId)
}

function Invoke-WithTemporaryFileWriteAccess {
    param(
        [string]$Path,
    ...
    try {
        try {
            $writableAcl = Get-Acl -LiteralPath $Path
            $writableAcl.SetAccessRule($rule)
            Set-Acl -LiteralPath $Path -AclObject $writableAcl
        }
        catch {
            & takeown.exe /f $Path /a | Out-Null
            if ($LASTEXITCODE -ne 0) {
                throw "无法临时取得文件所有权: $Path"
            }
            $writableAcl = Get-Acl -LiteralPath $Path
            $writableAcl.SetAccessRule($rule)
            Set-Acl -LiteralPath $Path -AclObject $writableAcl
        ...
        # A delete operation may intentionally remove the file. There is then no
        # object whose ACL needs restoring; the parent directory ACL was untouched.
...
function Invoke-WithTemporaryDirectoryWriteAccess {
    param(
        [string]$Path,
        [scriptblock]$Operation
    )
...
function Restore-LatestBackup {
    ...
    foreach ($file in $files) {
        ...
        if (-not (Test-Path -LiteralPath $parent -PathType Container)) {
            throw "备份对应的官方目录不存在，拒绝跨版本恢复: $parent"
        }
        Invoke-WithTemporaryDirectoryWriteAccess $parent {
            Invoke-WithTemporaryFileWriteAccess $target {
...
function Install-LanguageFiles {
    ...
    if (-not (Test-Path -LiteralPath $i18nDir -PathType Container)) {
        Invoke-WithTemporaryDirectoryWriteAccess $ResourcesPath {
            New-Item -ItemType Directory -Path $i18nDir | Out-Null
        }
    }
    if (-not (Test-Path -LiteralPath $statsigDir -PathType Container)) {
        Invoke-WithTemporaryDirectoryWriteAccess $i18nDir {
            New-Item -ItemType Directory -Path $statsigDir | Out-Null
        }
    }

    $frontendEnglish = Join-Path $i18nDir "en-US.json"
    $frontendTarget = Join-Path $i18nDir "$Lang.json"
    $frontendBase = Get-Content -LiteralPath $frontendEnglish -Raw -Encoding UTF8 | ConvertFrom-Json
    $frontendPack = Get-Content -LiteralPath $Pack["Frontend"] -Raw -Encoding UTF8 | ConvertFrom-Json
    $frontendTranslated = 0
    foreach ($property in $frontendPack.PSObject.Properties) {
        $targetProperty = $frontendBase.PSObject.Properties[$property.Name]
        if ($null -ne $targetProperty) {
            $targetProperty.Value = $property.Value
            $frontendTranslated += 1
        }
    }
    $frontendJson = $frontendBase | ConvertTo-Json -Depth 100
    Invoke-WithTemporaryDirectoryWriteAccess $i18nDir {
        Invoke-WithTemporaryFileWriteAccess $frontendTarget {
            [System.IO.File]::WriteAllText($frontendTarget, $frontendJson, $Utf8NoBom)
        }
    }
    Write-Host "  installed ion-dist/i18n/$Lang.json ($frontendTranslated translated; current English retained for the rest)" -ForegroundColor Green

    $desktopEnglish = Join-Path $ResourcesPath "en-US.json"
    $desktopTarget = Join-Path $ResourcesPath "$Lang.json"
    $desktopBase = Get-Content -LiteralPath $desktopEnglish -Raw -Encoding UTF8 | ConvertFrom-Json
    $desktopPack = Get-Content -LiteralPath $Pack["Desktop"] -Raw -Encoding UTF8 | ConvertFrom-Json
    $desktopTranslated = 0
    foreach ($property in $desktopPack.PSObject.Properties) {
        $targetProperty = $desktopBase.PSObject.Properties[$property.Name]
        if ($null -ne $targetProperty) {
            $targetProperty.Value = $property.Value
            $desktopTranslated += 1
        }
    }
    $desktopJson = $desktopBase | ConvertTo-Json -Depth 100
    Invoke-WithTemporaryDirectoryWriteAccess $ResourcesPath {
        Invoke-WithTemporaryFileWriteAccess $desktopTarget {
            [System.IO.File]::WriteAllText($desktopTarget, $desktopJson, $Utf8NoBom)
        }
    }
    Write-Host "  installed resources/$Lang.json ($desktopTranslated translated; current English retained for the rest)" -ForegroundColor Green

    $statsigTarget = Join-Path $statsigDir "$Lang.json"
    Invoke-WithTemporaryDirectoryWriteAccess $statsigDir {
        Invoke-WithTemporaryFileWriteAccess $statsigTarget {
            Copy-Item $Pack["Statsig"] $statsigTarget -Force
        }
    }
...
function Register-Language {
    ...
    foreach ($file in $jsFiles) {
        ...
        if ($regex.IsMatch($text)) {
            ...
            Invoke-WithTemporaryFileWriteAccess $file.FullName {
...
function Patch-LanguageDisplayNames {
    ...
    $languageList = "$BaseLanguageList,`"zh-CN`"]"
    $patchedFiles = 0
    foreach ($file in $jsFiles) {
        $text = [System.IO.File]::ReadAllText($file.FullName, [System.Text.Encoding]::UTF8)
        ...
        if (-not $text.Contains($languageList)) {
            continue
        }
...
function Remove-LanguageFiles {
    ...
    if ((Test-Path -LiteralPath $statsigDir -PathType Container) -and
        @(Get-ChildItem -LiteralPath $statsigDir -Force -ErrorAction SilentlyContinue).Count -eq 0) {
        $i18nDir = Split-Path -Parent $statsigDir
        Invoke-WithTemporaryDirectoryWriteAccess $i18nDir {
            Remove-Item -LiteralPath $statsigDir -Force -ErrorAction SilentlyContinue
        }
    }

    if ($failed.Count -gt 0) {
        throw "仍有 $($failed.Count) 个中文资源文件未能删除；卸载未完成。"
    }
}
...
function Install-WindowsLanguagePack {
    ...
    try {
        ...
        Write-Step "[4/8] 验证现有写入权限"
        Write-Host "  using the existing resource-directory permissions; no ACL changes requested" -ForegroundColor Green
        ...
        Write-Step "[7/8] 保持官方前端 bundle"
        Write-Host "  skipping hardcoded JS text replacement to preserve 3P local-frontend syntax" -ForegroundColor Green
        Write-Host "  skipping language display-name injection; JSON localization remains enabled" -ForegroundColor Green
...
function Uninstall-WindowsLanguagePack {
    ...
    if (Test-AsarPatchEnabled) {
        ...
        Write-Host "  skipping Claude.exe asar integrity sync; safe mode never modified app.asar" -ForegroundColor Green
    }
    ...
    Write-Host ""
    Write-Host "卸载完成，Claude Desktop 已恢复并重启。" -ForegroundColor Green
}
